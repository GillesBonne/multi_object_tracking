...
def loss_conf_tensor(object_mask, pred_box_conf, true_box_conf, obj_scale, noobj_scale, conf_delta):
    ...
    conf_delta = object_mask_ * (pred_box_conf-true_box_conf) * \
        obj_scale + (1-object_mask_) * conf_delta * noobj_scale
...
def _create_mesh_xy(batch_size, grid_h, grid_w, n_box):
    ...
    # Returns
        mesh_xy : Tensor, shape of (batch_size, grid_h, grid_w, n_box, 2)
            ...
            [..., 1] means "grid_h"
    """
    mesh_x = tf.cast(tf.reshape(tf.tile(tf.range(grid_w), [
                     grid_h]), (1, grid_h, grid_w, 1, 1)), tf.float32)
