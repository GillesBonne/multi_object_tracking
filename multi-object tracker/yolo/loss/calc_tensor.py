...
from yolo.loss.utils import (adjust_pred_tensor, adjust_true_tensor,
                             conf_delta_tensor, loss_class_tensor,
                             loss_conf_tensor, loss_coord_tensor,
                             wh_scale_tensor)
...
class LossTensorCalculator(object):
    ...
    def run(self, y_true, y_pred, anchors=[90, 95, 92, 154, 139, 281]):
        ...
        loss_box = loss_coord_tensor(
            object_mask, preds[..., :4], trues[..., :4], wh_scale, self.xywh_scale)
        loss_conf = loss_conf_tensor(
            object_mask, preds[..., 4], trues[..., 4], self.obj_scale, self.noobj_scale, conf_delta)
