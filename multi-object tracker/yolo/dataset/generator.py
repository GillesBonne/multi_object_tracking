...
class BatchGenerator(object):
    ...
    def _get(self):
        ...
        fname, boxes, coded_labels = parse_annotation(
            self.ann_fnames[self._index], self.img_dir, self.lable_names)
        ...
        for original_box, label in zip(boxes_, coded_labels):
            ...
            _coded_box = _encode_box(list_ys[scale_index],
                                     original_box, max_anchor, net_size, net_size)
...
def _create_empty_xy(net_size, n_classes, n_boxes=3):
    ...
    ys_1 = np.zeros((1*base_grid_h,  1*base_grid_w, n_boxes,
                     4+1+n_classes))  # desired network output 1
    ys_2 = np.zeros((2*base_grid_h,  2*base_grid_w, n_boxes,
                     4+1+n_classes))  # desired network output 2
    ys_3 = np.zeros((4*base_grid_h,  4*base_grid_w, n_boxes,
                     4+1+n_classes))  # desired network output 3
